#include "defines.h"
#include "http_server.h"
#include "pressureSystem.h"
#include "vibration.h"

#ifdef INFLUXDB
InfluxDBClient client(INFLUXDB_URL, INFLUXDB_ORG, INFLUXDB_BUCKET, INFLUXDB_TOKEN, InfluxDbCloud2CACert);
#ifdef LEFT
Point sensorStatus("L");
#else
Point sensorStatus("R");
#endif
#endif

void classifyMovement() {
    if (sensorValueH > baselineH + 60) {
        sState = GROUND;
    } else if (sensorValueH > baselineH + 20) {
        sState = STANDING;
    } else if (false) {
        sState = UP;
    } else {
        sState = UNDEFINED;
    }
}

void TaskUpload(void *pvParameters) {
    (void)pvParameters;
    esp_task_wdt_init(30, false);
    print("Start uploadtask", DEBUG_INFO_LVL);
    SensorReading item;
    long tempUploadTime = 0;
    int connectionFails = 0;

#ifdef INFLUXDB
    client.setHTTPOptions(HTTPOptions().connectionReuse(true).httpReadTimeout(2000));
    client.setWriteOptions(WriteOptions().writePrecision(WRITE_PRECISION).batchSize(MAX_BATCH_SIZE).bufferSize(WRITE_BUFFER_SIZE).retryInterval(1000));

    for (;;) {
        // Check connection BEFORE collecting data
        if (!client.validateConnection()) {
            connectionFails++;
            print("InfluxDB connection failed!", DEBUG_ERROR_LVL);
            printS(client.getLastErrorMessage(), DEBUG_ERROR_LVL);
            if (connectionFails >= 5) {
                print("Too many connection failures. Clearing buffer.", DEBUG_ERROR_LVL);
                client.resetBuffer();
                connectionFails = 0;
            }
            vTaskDelay(5000 / portTICK_PERIOD_MS);
            continue;
        }

        connectionFails = 0;

        int xD = 0;
        while (xQueueReceive(msg_queue, (void *)&item, 0) == pdTRUE && xD < MAX_BATCH_SIZE) {
            xD++;
            sensorStatus.clearFields();
            sensorStatus.addField("ax", item.ax);
            sensorStatus.addField("ay", item.ay);
            sensorStatus.addField("az", item.az);
            sensorStatus.addField("gx", item.gx);
            sensorStatus.addField("gy", item.gy);
            sensorStatus.addField("gz", item.gz);
            sensorStatus.addField("pV", item.pressureFront);
            sensorStatus.addField("pH", item.pressureBack);
            sensorStatus.addField("pB", item.pressureBaseline);
            sensorStatus.setTime(item.time_stamp);
            client.writePoint(sensorStatus);
        }

        lastQueLen = xD;
        printS(String("Start Upload von : ") + String(xD), DEBUG_DEBUG_LVL);
        tempUploadTime = micros();

        Serial.printf("Free heap: %u\n", ESP.getFreeHeap());
        Serial.printf("Min heap: %u\n", ESP.getMinFreeHeap());
        Serial.printf("Max alloc heap: %u\n", ESP.getMaxAllocHeap());

        if (!client.flushBuffer()) {
            print("InfluxDB flush failed: ", DEBUG_ERROR_LVL);
            printS(client.getLastErrorMessage(), DEBUG_ERROR_LVL);
            print("Full buffer: ", DEBUG_ERROR_LVL);
            print(client.isBufferFull() ? "Yes" : "No", DEBUG_ERROR_LVL);
            vTaskDelay(3000 / portTICK_PERIOD_MS);
            continue;
        }

        lastUpload = micros() - tempUploadTime;
        print("END Uploadet", DEBUG_DEBUG_LVL);
    }
#endif
}

void TaskLoop(void *pvParameters) {
    (void)pvParameters;
    esp_task_wdt_init(30, false);
    print("Start Loop", DEBUG_INFO_LVL);
    long tempMessure = 0;
    struct timeval tv;
    TickType_t xLastWakeTime = xTaskGetTickCount();

    for (;;) {
        vTaskDelayUntil(&xLastWakeTime, MESSUREMENT_SMAPLE_TIME / portTICK_PERIOD_MS);
        tempMessure = micros();
        gettimeofday(&tv, NULL);
        recordAccelRegisters();
        recordGyroRegisters();
        messure();
        classifyMovement();
        updateValveSystem();
        if (vibrate) updateVibrationSystem();

#ifdef INFLUXDB
        SensorReading pointValue = {accelX, accelY, accelZ, gyroX, gyroY, gyroZ, sensorValueV, sensorValueH, baselineH, getTimeStamp(&tv, 3)};
        if (xQueueSend(msg_queue, (void *)&pointValue, 0) != pdTRUE) {
            print("Queue full", DEBUG_ERROR_LVL);
            vTaskDelay(1000 / portTICK_PERIOD_MS); // statt reboot
            continue;
        }
#endif

        lastMesurement = micros() - tempMessure;
        printS("Die Messung hat " + String(lastMesurement) + " Microsekunden gedauert", DEBUG_DEBUG_LVL);
    }
}

void setup() {
    Serial.begin(115200);
    Wire.begin();
    Wire.setClock(400000);
    setupMPU();

#ifdef WiFi_EN
    bool res;
#ifdef LEFT
    res = wm.autoConnect("ShoeL", "shoeshoe");
#else
    res = wm.autoConnect("ShoeR", "shoeshoe");
#endif
    printS(String(WiFi.localIP()), DEBUG_INFO_LVL);
#ifdef INFLUXDB
    timeSync(TZ_INFO, NTP_SERVER1, NTP_SERVER2);
#endif
#endif

    print("time Fertig", DEBUG_INFO_LVL);
    if (queueStorageArea == NULL || queueDataStructure == NULL) {
        print("Failed to allocate memory for the queue in (PS)RAM", DEBUG_ERROR_LVL);
        while (1);
    }

    msg_queue = xQueueCreateStatic(msg_queue_len, sizeof(SensorReading), queueStorageArea, queueDataStructure);
    if (msg_queue == NULL) {
        print("Failed to create the queue", DEBUG_ERROR_LVL);
        while (1);
    } else {
        print("Queue successfully created in (PS)RAM", DEBUG_INFO_LVL);
    }

    print("Queue Fertig", DEBUG_INFO_LVL);
    init_vibration();
    init_pressureSystem();
    print("pins Fertig", DEBUG_INFO_LVL);
    vTaskDelay(50 / portTICK_PERIOD_MS);
    calibrate();
    print("callibarate Fertig", DEBUG_INFO_LVL);

    xTaskCreatePinnedToCore(TaskLoop, "TaskLoop", ramLoop, NULL, configMAX_PRIORITIES - 1, &tloop, ARDUINO_RUNNING_CORE);
    print("task1 Fertig", DEBUG_INFO_LVL);

#ifdef INFLUXDB
    vTaskDelay(10 / portTICK_PERIOD_MS);
    xTaskCreatePinnedToCore(TaskUpload, "TaskUpload", ramUpload, NULL, 1, &tUpload, ARDUINO_RUNNING_CORE);
#endif

#ifdef WiFi_EN
    xTaskCreatePinnedToCore(TaskWebserver, "TaskWebserver", ramWebservice, NULL, 1, NULL, ARDUINO_RUNNING_CORE);
#endif
    print("Webserver Fertig", DEBUG_INFO_LVL);
}

void loop() {
    esp_task_wdt_init(30, false);
    vTaskDelete(NULL);
}